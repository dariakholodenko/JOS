...
trap_init(void)
{
	...
	SETGATE(idt[T_DEFAULT], false, GD_KT, t_default, 0); 
	
	/* IRQ */
	//set all unsupported irqs to a default irq_error handler
	int i;
	for (i = 2; i < 16; i++) {
		if (i != IRQ_SERIAL && 
			i != IRQ_SPURIOUS && 
	...
	SETGATE(idt[IRQ_OFFSET + IRQ_IDE], false, GD_KT, irq_ide, 0);	
...
trap_init_percpu(void)
{
	...
	thiscpu->cpu_ts.ts_esp0 = (uint32_t)percpu_kstacks[cpu_id] + KSTKSIZE; //KSTACKTOP - cpu_id * (KSTKSIZE + KSTKGAP);
...
trap_dispatch(struct Trapframe *tf)
{
	...
	if (tf->tf_trapno == T_PGFLT) {
		if (debug)
			cprintf("[%08x] page fault trap from eip %08x\n", curenv->env_id, tf->tf_eip);
	...
	if (tf->tf_trapno == T_SYSCALL) {
		uint32_t retval = syscall(tf->tf_regs.reg_eax, 
				tf->tf_regs.reg_edx, 
				tf->tf_regs.reg_ecx,
				tf->tf_regs.reg_ebx,
				tf->tf_regs.reg_edi,
	...
	if (tf->tf_cs == GD_KT)
		...
		cprintf("[%08x] Unexpected trap %d\n", curenv->env_id, tf->tf_trapno);
		env_destroy(curenv);
		return;
	}
}
...
trap(struct Trapframe *tf)
{
	...
	if (read_eflags() & FL_IF) {
		cprintf("%s:%d  Trap: %d\n", __FILE__, __LINE__, tf->tf_trapno);
		cprintf("%s:%d  EFLAGS: %d\n", __FILE__, __LINE__, tf->tf_eflags);
		print_trapframe(tf);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	//check if CPL(two lower bits) is kernel(kernel PL = 0)
	if ((tf->tf_cs & 3) == 0)
	...
	if (curenv->env_pgfault_upcall) {	
		...
		if (curenv->env_tf.tf_esp >= UXSTACKTOP - PGSIZE && 
			curenv->env_tf.tf_esp < UXSTACKTOP) {
				//-4 is for an empty word
				tftop = curenv->env_tf.tf_esp - 4;
		}
		
		//- sizeof(struct UTrapframe) is to allocate place for utf on stack
		struct UTrapframe *utf = (struct UTrapframe*)(tftop - sizeof(struct UTrapframe));
		
		//check that the exception stack is present and writable 
		//and the exception doesn't stack overflow
		...
		utf->utf_esp = tf->tf_esp; //the trap-time stack to return to
	...
	cprintf("[%08x] page_fault_handler: user fault va %08x ip %08x\n",
