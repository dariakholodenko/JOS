...
spawn(const char *prog, const char **argv)
{
	...
	fd = r;
	if (debug)
		cprintf("%s %d: envid %08x: fd = %d, prog = %s\n",__FILE__, __LINE__, sys_getenvid(), fd, prog);
	// Read elf header
...
copy_shared_pages(envid_t child)
{
	...
	for (ptx = PGNUM(UTEXT); ptx < PGNUM(UXSTACKTOP - PGSIZE); ptx++) {
		...
		if ((uvpd[pdx] & PTE_P) && (uvpt[ptx] & PTE_P)) { 
			int perm = PGOFF(uvpt[ptx]) & PTE_SYSCALL;
			if (perm & PTE_SHARE) { 
				uintptr_t va = ptx*PGSIZE;
	...
	for(addr = 0; addr < USTACKTOP; addr += PGSIZE) {
		if ((uvpd[PDX(addr)] & PTE_P) && (uvpt[PGNUM(addr)] & PTE_P)) {
			...
			if (perm & PTE_SHARE) {
				if ((r = sys_page_map(parent_envid, (void *)addr, 
